...
function implementationCardHTML({ actor, item, runtime, resource, richData, undone = false }) {
  ...
  return `<div class="fast-nri-ability-use-card ${undone ? "resource-undone" : ""}">
    ...
    ${richData.profiles.length ? `<div class="fast-nri-ability-profile-summary">${richData.profiles.map(profile => `<section class="fast-nri-chat-degree-profile"><strong>${esc(profile.label)}</strong><div class="fast-nri-chat-profile-formulas">${profile.damage}${profile.healing}${profile.tempHp}</div>${profile.text ? `<div class="fast-nri-chat-profile-text">${profile.text}</div>` : ""}${profile.effects.join("")}</section>`).join("")}</div>` : ""}
